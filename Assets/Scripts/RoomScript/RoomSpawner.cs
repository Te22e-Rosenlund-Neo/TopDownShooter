...
if(other.GetComponent<RoomSpawner>().OpeningDirection == 0){
    Destroy(gameObject);
}else{
    if(other.GetComponent<RoomSpawner>().HasSpawned == false && HasSpawned == false && transform.position.x != 0 && transform.position.y != 0){
